...
pub fn setup_table(lua: &Lua) -> LuaResult<LuaTable> {
    ...
    exports.set("with_transporter", lua.create_function(|_, t: AnyUserData| {
        let transporter = t.take::<LuaCC>()?;

        Ok(ConsumerSetupLua::<dyn CommunicationConsumer> { phantom: Default::default() } )
    })?)?;
